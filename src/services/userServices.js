...
export const sendEmailForRecover = async (email) => {
	try {
		const response = await fetch(`${URL_API_BACKEND}/resetPassword`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
...
export const changePassword = async (data) => {
	...
	try {
		const response = await fetch(`${URL_API_BACKEND}/changePassword`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
